...
- **Role gating:** Prefer decoding the user's role from the **JWT access-token claims** client-side (no extra round-trip) IF the token carries a role/authorities claim — the researcher must verify this against how the backend issues tokens; fall back to a lightweight profile/me read only if the claim is absent. Gate ADMIN-only controls by **hiding** them for STAFF (cleaner than disabled+tooltip). ADMIN-only areas per `SecurityConfig.java`: **Menu catalog CRUD and Table catalog CRUD (areas/tables) only**. CORRECTION (verified in 20-UI-SPEC.md §6 against SecurityConfig): `/admin/inventory/**`, `/admin/menu/recipes/cost`, and `/admin/menu/costing` are matched by an earlier `hasAnyRole("ADMIN","STAFF")` rule before the `/admin/**`→ADMIN catch-all — so **Inventory controls stay VISIBLE to STAFF**, not hidden. STAFF-allowed areas (table-ops, payments, kitchen, order cancellation, inventory) keep full controls. JWT carries a `roles` claim (`JwtProvider`) → decode client-side, no extra endpoint. **20-UI-SPEC.md §6 is authoritative for the role matrix.**
