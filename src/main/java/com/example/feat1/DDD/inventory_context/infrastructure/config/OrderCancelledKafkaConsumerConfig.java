...
/**
 * Consumer wiring for the Inventory context's {@code OrderCancelled} listener (CANCEL-05). Mirrors
 * {@link SettleTriggerKafkaConsumerConfig} bean-for-bean — RESEARCH Pattern 1 (typed {@link
 ...
 * InventoryKafkaConsumerConfig} for the whole context. This class only adds the
 * order-cancelled-release triplet and reuses the existing {@code inventoryDltKafkaTemplate} for DLT
 * republishing (no new DLT template — inventory already has one).
 *
 * <p>Serde uses the phase-wide Jackson-3 {@link JacksonJsonDeserializer}: {@code
 * OrderCancelledEvent} carries a {@code java.time.Instant} the legacy Jackson-2 deserializer cannot
 ...
 * DLT instead of blocking the partition (T-18-03-03). {@code USE_TYPE_INFO_HEADERS=false} + a fixed
 * {@code VALUE_DEFAULT_TYPE} plus a {@code TRUSTED_PACKAGES} allow-list pinned to the OWNING
 * context ({@code order_context.application.event}, this event class is cross-context) defeat
 * malicious {@code __TypeId__} headers.
 */
@Configuration
public class OrderCancelledKafkaConsumerConfig {
  ...
  public ConsumerFactory<String, OrderCancelledEvent> orderCancelledInventoryConsumerFactory(
    ...
    // CROSS-CONTEXT: OrderCancelledEvent lives in order_context, not inventory_context.
    props.put(
    ...
    // Ignore the producer __TypeId__ header and force the local default type.
  ...
  public DefaultErrorHandler orderCancelledInventoryErrorHandler(
    ...
    // A deserialization failure is a poison pill: do not retry, route straight to the DLT.
    // A missing reservation/order-line is intentionally left retryable so a transient
    // cancel-before-reserve ordering race self-heals via retry before hitting the DLT.
  ...
  public ConcurrentKafkaListenerContainerFactory<String, OrderCancelledEvent>
      orderCancelledInventoryKafkaListenerContainerFactory(
          ConsumerFactory<String, OrderCancelledEvent> orderCancelledInventoryConsumerFactory,
          DefaultErrorHandler orderCancelledInventoryErrorHandler) {
    ConcurrentKafkaListenerContainerFactory<String, OrderCancelledEvent> factory =
        new ConcurrentKafkaListenerContainerFactory<>();
    factory.setConsumerFactory(orderCancelledInventoryConsumerFactory);
    factory.setCommonErrorHandler(orderCancelledInventoryErrorHandler);
