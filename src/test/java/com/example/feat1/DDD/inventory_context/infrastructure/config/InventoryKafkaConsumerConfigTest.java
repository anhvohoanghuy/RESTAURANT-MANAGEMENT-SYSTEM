...
/**
 * Broker-free wiring test for {@link InventoryKafkaConsumerConfig}: instantiates the config
 * directly (no Spring context, no live broker) and asserts the consumer factory, container factory,
 * and error-handler beans are wired for at-least-once + poison-pill safety (D-04).
 */
class InventoryKafkaConsumerConfigTest {

  private static final String BOOTSTRAP = "localhost:9092";
  private static final String GROUP_ID = "inventory-order-created";

  private final InventoryKafkaConsumerConfig config = new InventoryKafkaConsumerConfig();

  @Test
  void consumerFactoryDisablesAutoCommitAndForcesSafeDeserialization() {
    ConsumerFactory<String, OrderCreatedEvent> cf =
        config.orderCreatedConsumerFactory(BOOTSTRAP, GROUP_ID);
    ...
    // Type-confusion defence (T-15-07): ignore producer headers, force the local default type.
  ...
  void containerFactoryUsesRecordAckMode() {
    ...
    DefaultErrorHandler handler =
        config.orderCreatedErrorHandler(config.inventoryDltKafkaTemplate(BOOTSTRAP));

    ConcurrentKafkaListenerContainerFactory<String, OrderCreatedEvent> factory =
        config.orderCreatedKafkaListenerContainerFactory(cf, handler);
  ...
  void errorHandlerMarksDeserializationExceptionNotRetryable() {
    KafkaTemplate<String, Object> dlt = config.inventoryDltKafkaTemplate(BOOTSTRAP);
    assertThat(dlt).isNotNull();

    DefaultErrorHandler handler = config.orderCreatedErrorHandler(dlt);
