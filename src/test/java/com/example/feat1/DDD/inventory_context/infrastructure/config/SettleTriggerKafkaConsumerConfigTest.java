...
/**
 * Broker-free wiring test for {@link SettleTriggerKafkaConsumerConfig}: instantiates the config
 * directly (no Spring context, no live broker) and asserts the consumer factory, container factory,
 * and error-handler beans are wired for at-least-once + poison-pill safety, mirroring the Phase 15
 * hardening (D-01/D-05). The DLT template is reused from {@link InventoryKafkaConsumerConfig}.
 */
class SettleTriggerKafkaConsumerConfigTest {

  private static final String BOOTSTRAP = "localhost:9092";
  private static final String GROUP_ID = "inventory-settlement";

  private final SettleTriggerKafkaConsumerConfig config = new SettleTriggerKafkaConsumerConfig();
  private final InventoryKafkaConsumerConfig dltConfig = new InventoryKafkaConsumerConfig();

  @Test
  void consumerFactoryDisablesAutoCommitAndForcesSafeDeserialization() {
    ConsumerFactory<String, SettleTriggerEvent> cf =
        config.settleTriggerConsumerFactory(BOOTSTRAP, GROUP_ID);
    ...
    // Type-confusion defence (T-16-12): ignore producer headers, force the local default type.
    ...
    assertThat(props.get(JacksonJsonDeserializer.VALUE_DEFAULT_TYPE))
        .isEqualTo(SettleTriggerEvent.class.getName());
    assertThat(props.get(JacksonJsonDeserializer.TRUSTED_PACKAGES))
        .isEqualTo("com.example.feat1.DDD.inventory_context.application.event");
  }
  ...
  void containerFactoryUsesRecordAckMode() {
    ...
    DefaultErrorHandler handler =
        config.settleTriggerErrorHandler(dltConfig.inventoryDltKafkaTemplate(BOOTSTRAP));

    ConcurrentKafkaListenerContainerFactory<String, SettleTriggerEvent> factory =
        config.settleTriggerKafkaListenerContainerFactory(cf, handler);
  ...
  void errorHandlerMarksDeserializationExceptionNotRetryable() {
    KafkaTemplate<String, Object> dlt = dltConfig.inventoryDltKafkaTemplate(BOOTSTRAP);
    assertThat(dlt).isNotNull();

    DefaultErrorHandler handler = config.settleTriggerErrorHandler(dlt);
